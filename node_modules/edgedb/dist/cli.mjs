...
async function main(args) {
    ...
    const cliLocation = maybeCachedCliLocation ??
...
async function getCliLocationFromCache() {
    debug("Checking CLI cache...");
    try {
        let cachedBinaryPath = null;
        try {
            cachedBinaryPath = (await fs.readFile(CLI_LOCATION_CACHE_FILE_PATH, { encoding: "utf8" })).trim();
        }
        catch (err) {
            if (typeof err === "object" &&
                err !== null &&
...
async function getCliLocationFromTempCli() {
...
async function writeCliLocationToCache(cliLocation) {
...
async function selfInstallFromTempCli() {
...
function runEdgeDbCli(args, pathToCli, execOptions = { stdio: "inherit" }) {
...
async function findPackage() {
    ...
    if (!pkg) {
        throw Error(`No compatible EdgeDB CLI package found for the current platform ${dist}`);
    }
